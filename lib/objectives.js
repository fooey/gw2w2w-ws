        ...
        function(objective, nextObjective){
            ...
            async.each(
                ...
                function(lang, nextLang){
                    let commonName = Controller.getCommonName(lang.key, objective.id);

                    const useExpandedCampName = (
                        objectives[objective.id].type === 4
                        && (
                            objectives[objective.id].name === "Crossroads"
                            || objectives[objective.id].name === "Quarry"
                            || objectives[objective.id].name === "Lumber Mill"
                        )
                    );

                    if(useExpandedCampName){
                        commonName = objectives[objective.id].name + ': ' + commonName;
                    }
                    objectives[objective.id].commonNames[lang.key] = commonName;
                    nextLang();
                },
                nextObjective
            );
