    ...
    async.each(
        ...
        function(worldData, nextWorld){
            ...
            if(!worlds[worldData.id]){
                worlds[worldData.id] = {
                    id: worldData.id,
                    region: __getRegion(worldData.id),
                };
            }

            const slug = __getSlug(worldData.name);

            let world = {
                name: worldData.name,
                slug: slug,
                link: __getLink(worldData.lang, slug),
            };
...
function __getSlug(worldName){
    return require('slug')(worldName.toLowerCase()).replace('\'', '');
}


function __getLink(lang, slug){
    return ['/', lang, '/', slug].join('');
}


function __getRegion(worldId){
