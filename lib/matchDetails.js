...
const findUpdates = function(prevDetails, curDetails, findUpdatesCallback){
    ...
    async.parallel([
        ...
        //     findUpdatedScores(matchId, curMatch.scores, prevMatch.scores, nextUpdateFinder);
        // },
        function(nextUpdateFinder){
...
const processDiff = function (matchId, diff, objectiveId, callback){
    ...
    if(diffPath === 'owner'){
        ...
        if(diffKind === 'D'){
            wsPacket = {event: 'dropClaimer', arguments: {objectiveId: objectiveId}};
        }
        else{
            const guildId = diff.item.rhs;
            wsPacket = {event: 'newClaimer', arguments: {objectiveId: objectiveId, guild: guildId}};
        }
    }
    else{
        wsPacket = {event: 'unhandled', arguments: {objectiveId: objectiveId, diff: diff}};
    }
